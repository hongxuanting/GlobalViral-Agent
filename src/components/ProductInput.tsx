  ...
  return (
    <section className="glass-card p-5">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-white">商品输入区</h2>
          <p className="mt-1 text-sm text-slate-400">输入商品基础信息，生成跨平台增长方案。</p>
        </div>
      </div>
      ...
      <button
        ...
        onClick={onGenerate}
        disabled={loading}
        className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-md bg-gradient-to-r from-fuchsia-500 via-cyan-400 to-emerald-400 px-4 text-sm font-semibold text-slate-950 shadow-glow transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-70"
      >
